...
mod1_1<- function()
	{# FABENS MODEL WITH RPMA EFFECT
	for(i in 1:N)
		{
		# MODEL		
		L2[i] <- (Linf[rpmaIndx[i]]-L1[i])*(1-exp(-k[rpmaIndx[i]]*dY[i]))+L1[i]
		LL2[i]<-log(L2[i])	 

		# LIKLIHOOD
		Y[i]~dlnorm(LL2[i],prec_obs[rpmaIndx[i]])
		}
	for(i in 1:3)
		{
		k[i]~dunif(0.001,0.2)
		Linf[i]~dunif(800,4000)
		# OBSERVATION
		sigma_obs[i]  ~ dunif (0, 10)	
		prec_obs[i] <-pow(sigma_obs[i],-2)		
		}
	}	
